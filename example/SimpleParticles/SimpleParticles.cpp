    ...
    class Game : public IGame {
        ...
        void PostInitialize() override{
            ...
            for(int i = 0; i < PARTICLE_COUNT; i++){
                ...
                float x = r * cos(theta) * GameEngine->GetWindowHeight() / GameEngine->GetWindowWidth();
                float y = r * sin(theta);
        ...
        void Update() override {
            customUniformBufferObject.deltaTime = GameEngine->GetDeltaTime() * 1000;
        ...
        void Record() override {
            GameEngine->DrawParticlesFromStorageBuffer(0, PARTICLE_COUNT);
            for(int i = 1; i < GameEngine->GetObjectSize(); i++) GameEngine->DrawObject(i);
            GameEngine->DrawTexts();
        }

        void RecordComputeCommandBuffer() override {
            GameEngine->ComputeDispatch(PARTICLE_COUNT/256, 1, 1);
