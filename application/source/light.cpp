...
//LightingUniformBufferObject CLightManager::m_lightingUBO; //use inline member, so no need to declare there
//std::vector<void*> CLightManager::m_lightingUniformBuffersMapped;//use inline member, so no need to declare there
void CLight::Update(float deltaTime, int currentFrame, Camera &mainCamera, Camera &lightCamera){
    ...
    //if(CGraphicsDescriptorManager::graphicsUniformTypes & GRAPHCIS_UNIFORMBUFFER_LIGHTING){
        ...
        CLightManager::m_lightingUBO.lights[m_light_id].lightCameraProj = lightCamera.matrices.projection;
        CLightManager::m_lightingUBO.lights[m_light_id].lightCameraView = lightCamera.matrices.view;

        //update intensity to ubo
        CLightManager::m_lightingUBO.lights[m_light_id].ambientIntensity = m_intensity[0];
        CLightManager::m_lightingUBO.lights[m_light_id].diffuseIntensity = m_intensity[1];
        CLightManager::m_lightingUBO.lights[m_light_id].specularIntensity = m_intensity[2];
        CLightManager::m_lightingUBO.lights[m_light_id].dimmerSwitch = m_intensity[3];
        
        //update camera to ubo
        CLightManager::m_lightingUBO.mainCameraPos = glm::vec4(mainCamera.Position, 0);

        //update light pos, direction and color
        CLightManager::m_lightingUBO.lights[m_light_id].lightPos = glm::vec4(m_position, 0);
        CLightManager::m_lightingUBO.lights[m_light_id].lightDir = glm::vec4(lightCamera.DirectionFront, 0);
        CLightManager::m_lightingUBO.lights[m_light_id].lightColor = glm::vec4(m_color, 1.0f);
  
        CLightManager::m_lightingUBO.lights[m_light_id].spotInnerAngle = cos(glm::radians(m_spotOuterAngle)); 
        CLightManager::m_lightingUBO.lights[m_light_id].spotOuterAngle = cos(glm::radians(m_spotInnerAngle));

        memcpy(CLightManager::m_lightingUniformBuffersMapped[currentFrame], &CLightManager::m_lightingUBO, sizeof(CLightManager::m_lightingUBO));
    //}
